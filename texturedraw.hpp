...
void initTextureMap() {
  ...
  delete texImage;
#ifdef __APPLE__
  char *path = "/Users/dannylau/Desktop/COMP4411-Modeler/fur.bmp";
#else
  char *path = "fur.bmp";
#endif
  texImage = readBMP(path, texImageWidth, texImageHeight);
  if (!texImage) {
...
void drawTextureBody(double x, double y, double z) {
  ...
  if (mds->m_rayFile) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, texImageWidth, texImageHeight, 0,
    ...
    glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE,
              GL_DECAL); // default is GL_MODULATE for interacting with light
